...
class ObstacleAvoidanceNode(Node):
    def __init__(self):
        ...
        self.target_points = [
            #场景一 
            # np.array([4, -1.5, 0]).reshape(-1, 1) , # 新添加的目标点
            # np.array([4.2, -0.5, 0]).reshape(-1, 1) , # 新添加的目标点
            # np.array([3.0, 2.5, 0]).reshape(-1, 1) , # 新添加的目标点
            # np.array([2.6, 2.5, 0]).reshape(-1, 1) , # 新添加的目标点
            # np.array([0, 0, 0]).reshape(-1, 1) , # 新添加的目标点
            # #场景二
            # np.array([4.5, 0, 0]).reshape(-1, 1) , # 新添加的目标点
            # np.array([4.8, 1.0, 0]).reshape(-1, 1) , # 新添加的目标点
            # np.array([3.0, 2.5, 0]).reshape(-1, 1) , # 新添加的目标点
            # np.array([2.6, 2.5, 0]).reshape(-1, 1) , # 新添加的目标点
            # np.array([0, 0, 0]).reshape(-1, 1) , # 新添加的目标点
            #场景三
    ...
    def setup_mpc(self):
        ...
        # Q = np.array([[150.0, 0.0, 0.0], [0.0, 150.0, 0.0], [0.0, 0.0, 0.005]])
        # Q = np.array([[15.0, 0.0, 0.0], [0.0, 15.0, 0.0], [0.0, 0.0, 0.0025]]) # x、y、theta权重
        Q = np.array([[15.0, 0.0, 0.0], [0.0, 15.0, 0.0], [0.0, 0.0, 0.0025]]) # x、y、theta权重
        # Q = np.array([[10.0, 0.0, 0.0], [0.0, 10.0, 0.0], [0.0, 0.0, 0.0025]]) # 尝试增加x、y权重，好像没用
    ...
    def control_loop(self):
        if self.current_target_index < len(self.target_points):
            xs = self.target_points[self.current_target_index]
            ...
            # distance_to_target = np.linalg.norm(x_idk - xs)
            # angle_to_target = abs(self.theta_real - xs[2])
            ...
            if distance_to_target < 0.05:
                ...
                if self.current_target_index < len(self.target_points):
                    ...
                    self.get_logger().info(f"Switching to next target: {next_target.flatten()}")
                else:
